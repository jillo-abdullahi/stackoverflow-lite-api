...
def user_signup():
    ...
    for user_id in existing_users:
        if user_info['username'].lower() == existing_users[user_id]['username'].lower():
            response = jsonify(
...
def user_login():
    ...
    for user_id in existing_users:
        if user_info["email"] == existing_users[user_id]["email"] and check_password_hash(existing_users[user_id]["password"], password):

            return jsonify({"user": existing_users[user_id], "message": "User login successful"}), 200
